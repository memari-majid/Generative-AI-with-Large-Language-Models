...
def process_and_store_embeddings_in_batches(vectorstore: PGVector, documents: Iterator[List[Document]]):
    ...
    vectorstore.embedding_function = embeddings  # Update the embedding function
...
def truncate_input(input_text: str, tokenizer, max_input_length: int) -> str:
    """
...
def main():
    ...
    if 'llm' not in st.session_state:
        try:
            with st.spinner("Initializing OpenAI GPT-4 model..."):
                llm = OpenAI(
                ...
                logging.info("LLM initialized and loaded into session state.")
        except Exception as e:
            st.error(f"Failed to initialize the OpenAI GPT-4 model: {e}")
            logging.error(f"Failed to initialize the OpenAI GPT-4 model: {e}")
            return
    else:
        llm = st.session_state['llm']
        logging.info("LLM loaded from session state.")
